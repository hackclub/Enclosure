...
async function fetchAirtableRecordByEmail(email) {
  ...
  try {
    ...
    const approved =
      ...
      approvedRaw !== null &&
      String(approvedRaw).toLowerCase() === AIRTABLE_APPROVAL_VALUE;
              ...
              if (u) {
                ...
                await db
                  ...
                  .values({
                    ...
                    reason: `Payout for project ${updated.id}`,
                    createdAt: new Date(),
                  });
...
app.get("/api/auth/callback", async (req, res) => {
  ...
  try {
    ...
    const verificationStatus =
      typeof identity.verification_status === "string" ? identity.verification_status : undefined;
    ...
    if (identityId && identityEmail) {
      ...
      const basePayload = {
        ...
        identityToken:
          typeof tokenJson.access_token === "string" ? tokenJson.access_token : existing[0]?.identityToken || null,
        refreshToken:
          typeof tokenJson.refresh_token === "string" ? tokenJson.refresh_token : existing[0]?.refreshToken || null,
      ...
      if (existing.length) {
        ...
        try {
          await upsertAirtableUser({
            ...
            slackId: basePayload.slackId,
            role: basePayload.role,
            banned: basePayload.banned,
          });
        } catch (e) {
  ...
  try {
    ...
    if (String(table).toLowerCase().includes("user")) {
      ...
      const existing = await db
        .select()
        .from(users)
        .where(eq(users.id, String(identityId)))
        .limit(1);
      const payload = {
        ...
        if (exists.length) {
          await db
            ...
            .set({
              ...
              shopItemId: shopItemId ? String(shopItemId) : null,
  ...
  try {
    ...
    try {
      const [createdOrder] = await db
        ...
        .returning();
      try {
        await upsertAirtableOrder({
          id: createdOrder.id ?? item.id,
          userId: user.id,
          shopItemId: String(item.id),
          amount: price,
          status: "pending",
    ...
    await db
      .insert(shopTransactions)
      .values({ userId: user.id, amount: String(-price), reason: `Purchase: ${item.title}`, createdAt: new Date() });
    await db
      .update(users)
      .set({ credits: String(next) })
