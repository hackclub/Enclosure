...
async function airtableFetch(path, opts = {}) {
  ...
  const res = await fetch(url + (opts.method === "GET" && opts.body ? `?${String(opts.body)}` : ""), {
    ...opts,
    headers,
  });
  if (!res.ok) {
...
export async function upsertAirtableUser(u) {
  ...
  try {
    ...
    const res = await fetch(
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(AIRTABLE_USERS_TABLE)}?filterByFormula=${formula}`,
      {
    ...
    if (Array.isArray(j.records) && j.records.length) {
      ...
      const patch = await fetch(
        `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(AIRTABLE_USERS_TABLE)}/${recId}`,
        {
          method: "PATCH",
...
export async function upsertAirtableOrder(o) {
  ...
  try {
    ...
    const res = await fetch(
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(AIRTABLE_ORDERS_TABLE)}?filterByFormula=${formula}`,
      {
        headers: { Authorization: `Bearer ${AIRTABLE_API_KEY}` },
      }
    );
    ...
    if (Array.isArray(j.records) && j.records.length) {
      const recId = j.records[0].id;
      const patch = await fetch(
        `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(AIRTABLE_ORDERS_TABLE)}/${recId}`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${AIRTABLE_API_KEY}`, "Content-Type": "application/json" },
          body: JSON.stringify({ fields }),
