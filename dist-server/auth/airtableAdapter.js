...
async function findTokenRecord(type, fieldName, value) {
...
function mapAirtableRecordToUser(rec) {
    if (!rec)
        return null;
    const f = rec.fields || {};
    return {
...
export function airtableAdapter() {
    return {
        ...
        async createSession(session) {
            const fields = { sessionToken: session.sessionToken, userId: session.userId, Type: 'session' };
            if (session.expires)
                fields.expires = session.expires;
        ...
        async linkAccount(userId, providerAccount) {
            ...
            const recId = rec.id;
            const fields = {};
            if (providerAccount.providerId)
                fields.identity_provider = providerAccount.providerId;
            if (providerAccount.providerAccountId)
                fields.identity_user_id = providerAccount.providerAccountId;
            if (providerAccount.access_token)
                fields.identityToken = providerAccount.access_token;
            if (providerAccount.refresh_token)
                fields.refreshToken = providerAccount.refresh_token;
            await airtableFetch(`${USER_TABLE}/${recId}`, { method: 'PATCH', body: JSON.stringify({ fields }) });
        },
        async unlinkAccount(userId, providerId) {
            ...
            if (!rec)
                return;
            const recId = rec.id;
            const fields = { identity_provider: null, identity_user_id: null, identityToken: null, refreshToken: null };
            await airtableFetch(`${USER_TABLE}/${recId}`, { method: 'PATCH', body: JSON.stringify({ fields }) });
        }
    };
}
